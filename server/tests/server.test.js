...
describe('GET/todos/:id',()=>{
  ...
  it('should not give data from other user',(done)=>{
    request(app)
...
describe('DELETE/todos/:id',(req,res)=>{
  ...
  it('should delete todo from other user',(done)=>{
...
describe('POST/users',()=>{
  ...
  it('should create a user',(done)=>{
    var email='example@gmail.com' //input
    var password='qwerty'  //input
...
describe('POST/users/login',()=>{
  ...
  //  _id="userTwoId",
    ...
    .expect((res)=>{
      expect(res.headers['x-auth']).toExist()
      // expect(res.body._id).toBe(users._id)
      // expect(res.body.email).toBe(users.email)
    })
    .end((err,res)=>{
      if(err){
        return done(err)
...
describe('DELETE/users/me/token',()=>{
  it('should remove auth token on logout',(done)=>{
    var x_auth = users[0].tokens[0].token
    request(app)
